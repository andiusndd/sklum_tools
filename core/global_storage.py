...
try:
    ...
    import logging
    logger = logging.getLogger("SKLUM_Global")
...
def get_config_dir():
    ...
    if not config_dir.exists():
        try:
            ...
            logger.error(f"SKLUM Tools: Failed to create config dir: {e}")
            return None
    return config_dir
...
def save_license_key_global(key):
    ...
    try:
        ...
        logger.info(f"Saved global license to: {file_path}")
    except Exception as e:
        logger.error(f"SKLUM Tools: Failed to save global license: {e}")
...
def load_license_key_global():
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            if key:
                 logger.info(f"Loaded global license from {file_path}")
            return key
    except Exception as e:
        logger.error(f"SKLUM Tools: Failed to load global license: {e}")
        return ""
